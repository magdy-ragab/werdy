MainWin = New qApp {
        ...
        win1 = new qwidget() {
			...
			trayMenu=new QSystemTrayIcon()
			{
				...
				setIcon( new QIcon(new QPixMap("images/kuran.png") ) )
				show()
...
/* main tab */			
			page1 = new qwidget() {
				ReadNoticLabel = new qLabel(page1)
				{
					settext("انت تشاهد هذه النافذة لمتابعة قراءة وردك اليومي") 
					setStyleSheet("background-color:rgb(255,255,255);")
				}
				snoozeBtn = new qPushButton(page1) {
					settext("تأجيل")
					setclickevent("SnoozeReading()")
					setStyleSheet("qproperty-icon: url('images/clock.png');font-size:7pt;border:1px solid #B4A971;color:#55490C; height:20px;background-image: url('images/fff.png');border-radius:5px; font-weight:bold;background-color:rgb(255,255,255);")
                }
				ReadNoticLabelSep = new qLabel(page1)
				{
					settext("") 
					setStyleSheet("background-color:rgb(255,255,255);")
				}
				...
				layoutTop = new qHBoxLayout(page1) {
					addwidget(snoozeBtn)
					addwidget(ReadNoticLabelSep)
					addwidget(ReadNoticLabel)
				}
...
/* settings tab */
			...
			win2 = new qwidget() {
				...
				WerdContReadBtn  = new qPushButton(win2) {
					...
					setStyleSheet("qproperty-icon: url('images/quran-transparent-bg.png');")
...
/* search */
			page4= new qwidget() {
				...
				searchtxtBox= new qLineEdit(page4) {
					setFixedHeight(30)
					
					setStyleSheet("font-family:Tahoma, Verdana, Segoe, sans-serif;border:2px solid #B4A971;background-image:url('images/fff.png');border-radius:5px; height:30px")
                }
				...
				table2 = new qTableWidget(page4) {
					...
					setHorizontalHeaderItem(3, new QTableWidgetItem("اﻵية") )	
					setselectionbehavior(QAbstractItemView_SelectRows)
